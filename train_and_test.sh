...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/algolisp_train_dc_model.py --seed 43 --use_dataset_len 6000 --max_epochs 25 --inv_temp 0.05 --nHoles 3 -k 50)
	...
	name=algolisp_6000v2 g-run bash train_and_test.sh --inner > run.txt & #can i do this??
fi
