...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/algolisp_train_dc_model.py --filter_depth 1 2 3 4 5 --max_epochs 14 --inv_temp 0.05 --nHoles 3 -k 100 --use_dc_grammar)
	...
	name=algolisp_filter5_k100_t05 g-run bash train_and_test.sh --inner > run.txt & #can i do this??
fi
