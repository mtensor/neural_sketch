...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/algolisp_train_dc_model.py --limit_data 0.2 --max_epochs 15 --inv_temp 0.05 --nHoles 3 -k 50 --use_dc_grammar)
	...
	# train model:
	RES_TRAIN=$(sbatch --parsable --dependency=afterok:$RES_DC:$RES_PRE -e 'train.out' -o 'train.out' execute_gpu.sh python train/main_supervised_algolisp.py --limit_data 0.2 --train_to_convergence --max_epochs 15 --use_dc_grammar './saved_models/algolisp_dc_model.p' --inv_temp 0.25 --nHoles 3 -k 50 )
	...
	name=algolisp_limit20_first_run g-run bash train_and_test.sh --inner > run.txt & #can i do this??
fi
