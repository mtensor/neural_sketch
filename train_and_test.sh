...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/algolisp_train_dc_model.py --filter_depth 1 2 3 4 5 6 7 --max_epochs 7 --inv_temp 0.05 --nHoles 3 -k 50 --use_dc_grammar)
	...
	name=algolisp_filter7_k50_t05 g-run bash train_and_test.sh --inner > run.txt & #can i do this??
fi
