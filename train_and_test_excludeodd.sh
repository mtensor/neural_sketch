...
if [[ "$@" == "--inner" ]]; then
	...
	#Only pretrain
	RES_PRE=$(sbatch --parsable -e 'pretrain.out' -o 'pretrain.out' execute_gpu.sh python train/main_supervised_algolisp.py --exclude_odd --pretrain --max_epochs 0 --max_pretrain_epochs 45 --train_to_convergence)
	echo "pretraining job: $RES_PRE"

	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/algolisp_train_dc_model.py --exclude_odd --max_epochs 25 --inv_temp 0.05 --nHoles 3 -k 50)
 	echo "dc model training job: $RES_DC"

	# train model:
	RES_TRAIN=$(sbatch --parsable --dependency=afterok:$RES_PRE -e 'train.out' -o 'train.out' execute_gpu.sh python train/main_supervised_algolisp.py --exclude_odd --train_to_convergence --max_epochs 45 --use_dc_grammar './saved_models/algolisp_dc_model.p' --inv_temp 0.25 --nHoles 3 -k 50 --use_timeout)
	...
	echo "eval even jobs"
	sbatch --dependency=afterok:$RES_TRAIN -e 'finalevaleven.out' -o 'finaleval.out' execute_public_cpu.sh python eval/evaluate_algolisp.py --even --n_test 635 --mdl 100 --queue --n_processes 44 --timeout 600 --max_to_check 20000 --resultsfile "results_model"

	sbatch --dependency=afterok:$RES_PRE -e 'finalevalrnneven.out' -o 'finalevalrnn.out' execute_public_cpu.sh python eval/evaluate_algolisp.py --even --n_test 635 --mdl 100 --model_path "./saved_models/algolisp_pretrained.p" --resultsfile "results_rnn_base" --queue --n_processes 44 --timeout 600 --max_to_check 20000
	
	sbatch --dependency=afterok:$RES_DC -e 'finalevaldceven.out' -o 'finalevaldc.out' execute_public_cpu.sh python eval/evaluate_algolisp.py --even --n_test 635 --mdl 100 --dc_baseline --resultsfile "results_dc_base" --queue --n_processes 44 --timeout 600 --max_to_check 20000
	...
	name=algolisp_excludeodd_full g-run bash train_and_test_excludeodd.sh --inner > run.txt & #can i do this??
fi
