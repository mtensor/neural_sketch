...
if [[ "$@" == "--inner" ]]; then
	...
	#Only pretrain
	# RES_PRE=$(sbatch --parsable -e 'pretrain.out' -o 'pretrain.out' execute_gpu.sh python train/main_supervised_algolisp.py --seed 43 --pretrain --max_epochs 0 --max_pretrain_epochs 45 --use_dataset_len 6000 --train_to_convergence)
	...
	# train model:
	RES_TRAIN=$(sbatch --parsable --dependency=afterok:$RES_DC -e 'train.out' -o 'train.out' execute_gpu.sh python train/main_supervised_algolisp.py --load_pretrained_model_path ../algolisp_full_dataset_1544644404692/saved_models/algolisp_pretrained.p --train_to_convergence --max_epochs 45 --use_dc_grammar './saved_models/algolisp_dc_model.p' --inv_temp 0.25 --nHoles 3 -k 50 --use_timeout)
	...
	name=algolisp_full2 g-run bash train_and_test_full.sh --inner > run.txt & #can i do this??
fi
