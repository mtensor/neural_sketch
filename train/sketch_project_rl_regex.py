...
if __name__ == "__main__":
    ...
    try:
        if args.start_with_holes:
            model=torch.load("./saved_models/sketch_model_pretrain_holes.p")
            print('found saved model, loading pretrained model with holes')
        else:
            model=torch.load("./saved_models/sketch_model_pretrained.p")
    ...
    if args.pretrain:
        ...
        for i in range(model.pretrain_iteration, 20000):
            ...
            if not args.nosave:
                if i%500==0: torch.save(model, '.saved_models/sketch_model.p')
