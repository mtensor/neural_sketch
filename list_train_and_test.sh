...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/deepcoder_train_dc_model.py --max_epochs 5 --inv_temp 0.05 --nHoles 3 -k 50 --improved_dc_model) #or not.. --use_dc_grammar
 	echo "dc model training job: $RES_DC"

	# train model:
	RES_TRAIN=$(sbatch --parsable --dependency=afterok:$RES_DC -e 'train.out' -o 'train.out' execute_gpu.sh python train/main_supervised_deepcoder.py --max_epochs 10 --use_dc_model './saved_models/list_dc_model.p' --improved_dc_model --inv_temp 0.25 --nHoles 3 -k 50 --use_timeout --timing --load_pretrained_model_path ../deepcoder_pretrained_V128_10_epochs_1536681569098/deepcoder_pretrained.p_9.p)


	# test model
	echo "Eval job:"
	sbatch --dependency=afterok:$RES_TRAIN -e 'eval.out' -o 'eval.out' execute_public_cpu.sh python eval/evaluate_deepcoder.py --max_to_check 20000 --dcModel --improved_dc_grammar
	...
	name=deepcoder_multihole_T3_experiment1 g-run bash list_train_and_test.sh --inner > run.txt & #can i do this??
fi
