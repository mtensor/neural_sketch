...
if [[ "$@" == "--inner" ]]; then
	...
	# train dc_model:
	RES_DC=$(sbatch --parsable -e 'dctrain.out' -o 'dctrain.out' execute_gpu.sh python train/robustfill_train_dc_model.py --max_iterations 1000000 --inv_temp 0.05 --nHoles 3 -k 50 --improved_dc_model) #or not.. --use_dc_grammar
 	echo "dc model training job: $RES_DC"

	# train model:
	RES_TRAIN=$(sbatch --parsable --dependency=afterok:$RES_DC -e 'train.out' -o 'train.out' execute_gpu.sh python train/main_supervised_robustfill.py --use_dc_grammar './saved_models/text_dc_model.p' --improved_dc_model --inv_temp 0.25 --nHoles 3 -k 50 --use_timeout --timing --max_iteration 20000  --load_pretrained_model_path ../rb_long_pretrain_1537123008935/robustfill_pretrained.p )

	# test model
	echo "Eval job:"
	sbatch --dependency=afterok:$RES_TRAIN -e 'eval.out' -o 'eval.out' execute_public_cpu.sh python eval/evaluate_robustfill.py --beam --n_samples 50 --max_to_check 20000 --dcModel --improved_dc_grammar
	...
	name=rb_multihole_1experiment g-run bash text_train_and_test.sh --inner > run.txt & #can i do this??
fi
